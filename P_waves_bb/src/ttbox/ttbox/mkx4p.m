...
%                 'distance': P is the epicentral distance in degrees to which the ray
%                             is sent. This applies only for Pdiff and
%                             Sdiff phases, where the covered distance is
%                             independent of the ray parameter.
%                             If ANGLEMODE is 'distance' and the phase is
%                             neither Pdiff nor Sdiff, an error will occur.
%                 DEFAULT: 'rayparm'
...
%                   04.12.2006, 12.11.2010
%
% LIMITATIONS: rays that go through the deepest layer are not allowed due to 
...
% 12.11.2010  Evaluation of Pdiff and Sdiff
       ...
       if isstr(p5)
          ...
          switch lower(p5)
             ...
             case {'angle'}
                 ...
                 resp=p;
             case {'distance'} % MK12112010
                 %%% p is an epicentral distance in degrees. It has to be
                 %%% checked that the phase is Pdiff or Sdiff, and the ray
                 %%% parameter for these phases has to be determined
                 
                 %%% store the requested distance
                 targetdistance=p;
                 
                 %%% throw an error if this mode is used with other phases
                 if (~strcmp(phase,'Pdiff'))&(~strcmp(phase,'Sdiff'))
                    error('MKX4P: if ANGLEMODE is DISTANCE, the phase has to be Pdiff or Sdiff!');
                 end; % if
                 
                 %%% determine the ray parameter of Pdiff or Sdiff
                 [prayp,srayp,newz]=mkdepth2rayp(model,model.cmb);
                 switch phase
                    case {'Pdiff'}
                         p=prayp(1);
                         basephase='P';
                 ...
                 %%% is the requested distance large enough?
                 %%% P diff and Sdiff only exist beyond the core shadow
                 %%% boundary. The distance to that is also the base
                 %%% distance to which CMB legs have to be added.
                 basedist=mkx4p(basephase,h,p,model,'rayparm');
                 if isnan(basedist)
                     %%% it was not possible to evaluate the ray with the given ray
                     %%% parameter. The reason probably is some slight
                     %%% numericalinnaccuracy that makes MKX4P believe that the ray
                     %%% penetrates the core. Therefore we have to search for a
                     %%% slightly different ray parameter to determine the basedist
                     oldangle=mkrayp2angle(basephase,h,p,model);
                     [newp,newangle]=mkfumblep(p,oldangle(end),'both',basephase,h,model);
                     basedist=mkx4p(basephase,h,newp,model,'rayparm');
                 end; % if isnan(basedist)
                 if targetdistance<=basedist
                    %%% if the requested distance is too short, the
                    %%% requested phase does not exist and NaN is returned.
                    tt=NaN;
                    return;
                 end; % if targetdistance
                 
                 %%% now the call can be treated like a call in "rayparm"
                 %%% mode! Just modify the mode and apply the same settings
                 %%% as in rayparm mode!
                 p5='rayparm';
                 resp=p;
                 takeoff=-inf;
...
%                   %%% end of trial fix MK01122006
      ...
      case{'Pdiff','Sdiff'} %%% Pdiff Sdiff %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         
         %%% switch between Pdiff and Sdiff by choosing the velocity
         switch phase
            case {'Pdiff'}
                 vflat=vpflat;
                 wavetypesymbol='P';
            case {'Sdiff'}
                 vflat=vsflat;
                 wavetypesymbol='S';
         end; % switch mode (inner switch
      
         %%% compute vertex depth
         vdep=modelsav.rp-modelsav.cmb; % fixed for Pdiff!!  vdep is a radius!
         if isnan(vdep)&(takeoff<90) % takeoff<90: mk26092003
            %%% NaN if no vertex exists
            %disp(['MKX4P: no vertex for ' phase ' at p=' num2str(p*pi/180) 's/deg. Returning NaN.']);
            dist=NaN;
            ...
            %%% compute distance from surface to real focus at HFLAT
            %disp('MKX4P: surface correction');
            [dist2,segx2,segz2]=mkxpsum(p,vflat,zflat,0,hflat,1); % novertex=1: suppress vertex code

            %%% depth correction for target distance
            targetdistdepthcorr=targetdistance+mkxflat2sfer(dist2,rp);

            %%% result construction is different for upgoing and downgoing rays
            if takeoff<=90 % MK11102006was: takeoff<90 % mk26092003
               %%% compute distance for surface focus
               %disp('MKX4P: surface focus');
               [dist1,segx1,segz1]=mkxpsum(p,vflat,zflat,0,vdepflat,0);
               %%% total epicentral distance of phase is twice the distance for surface focus
               %%% minus distance from surface to focal depth
               dist=targetdistance; %MK12112010, was: dist=mkxflat2sfer(2*dist1-dist2,rp);
               %%% ray geometry: several pieces have to be connected...
               %% first: transform anything into spherical world
               [dmy,segz1s]=mkflat2sfer(segz1,segz1,rp); % for surface-to-surface
               [dmy,segz2s]=mkflat2sfer(segz2,segz2,rp); % for focal depth correction
               segxs=cumsum(mkxflat2sfer(segx1,rp));  % for surface-to-surface
               segx2s=cumsum(mkxflat2sfer(segx2,rp)); % for focal depth correction
               %% first-and-half: add leg along CMB
               %%% there should be as many points per degree as in the
               %%% downgoing leg average!
               cmbpathlen=(targetdistdepthcorr-2*segxs(end)); % this arc length necessary along CMB
               segxcmb=unique([ceil(segxs(end)):(segxs(end)/length(segxs)):targetdistdepthcorr/2 targetdistdepthcorr/2]); % sample distances
               segzcmb=zeros(size(segxcmb))+segz1s(end); % sample depths
               segxs=[segxs segxcmb];
               segz1s=[segz1s segzcmb];
               %% second: build surface-to-surface ray
               indies=(length(segxs):-1:1);
               segx=[segxs 2*segxs(end)-segxs(indies)];% was: segx=[segxs 2*max(segxs)-segxs(indies)]; MK20022006
               segz=[segz1s segz1s(indies)];
               ...
               segtyp=char(zeros(1,length(segx)-1)+abs(wavetypesymbol));
            else
               %%% takeoff angle is larger then 90deg: ray goes from source upward
               %%% this happens only to P and depth phases (p-something, note
               %%% that for takeoff>90, PP becomes pP and is not handled here!)
