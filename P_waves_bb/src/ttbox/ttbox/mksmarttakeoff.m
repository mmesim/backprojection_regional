...
%                   12.10.2006, 13.10.2006, 16.11.2006, 04.10.2007,
%                   26.02.2008, 16.12.2009
%
% BUGS: if the velocity model does not define velocities for the core, the
%       routine is liekely to crash. (example: non-definition of Vs in JB
%       model)
...
% 04.10.2007: velocity-undefined-crash for nonexistent Vs in JB model
%             discovered, but not repaired.
% 25.02.2008: minangle and maxangle initialized as empty results
% 16.12.2009: if Vs==0,, the routine quits only if the phase begins with "S"
...
%%% Is Vs==0 at focus?
%%% If the ray begins as S, we quit, since this is not possible in a liquid
%%% MK16122009
if (focus.vs==0) & (phase(1)=='S')
   angles=[];
   return;
end; % if focus.vs==0
...
if ~isnan(imodel.icb)
   ...
   pindies=find(~isnan(picb));
   sindies=find(~isnan(sicb)); % this selection hopefully is the same as for P
   if (isempty(pindies))|(isempty(sindies))
       %%% ICB is undefined, so we cannot search for rays touching it
       %%% instead, we use the steepest possible rays MK28102011
       picb=psteep;
       sicb=ssteep;
   ...
   end; %if (isempty(pindies))|(isempty(sindies))
...
if ~isnan(imodel.icb)
    ...
    if (~isempty(icbindies))&(length(icbindies)>=2)
        ...
        %%% ICB is improperly defined, ray construction impossible.
        ...
        picbshallowp=NaN;
        sicbshallowp=NaN;
    end; % if (~isempty(icbindies))&(length(icbindies>=2))
