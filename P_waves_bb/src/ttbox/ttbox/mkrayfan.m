...
%
%                 IF the phase is either Pdiff or Sdiff, the angle list in
%                 DANGLE is taken as the list of desired epicentral
%                 distances (since Pdiff and Sdiff have the same ray parameter and
%                 take off angles at all distances)
%
...
% - the USERDATA field contains takeoff angle, maximum depth, epicentral
%   distance (in degrees) and ray path length (in km) in a cell array.
%
% Martin Knapmeyer 11.06.2002, 05.07.2002, 08.09.2003, 25.09.2003,
%                  21.11.2003, 25.02.2004, 10.10.2006, 12.10.2010,
%                  19.01.2011, 11.05.2011
%
...
switch nin
   case {3}
      ...
      if (~strcmp(phase,'Pdiff'))&(~strcmp(phase,'Sdiff'))
         %%% if phase is not Pdiff or Sdiff, ask for the ideal
         %%% range of take off angles
         angles=mksmarttakeoff(phase,model,h,dangle);
      else
         %%% if phase is either Pdiff or Sdiff, create a list
         %%% of target distances
         angles=0:dangle:180;
      end; % (~strcmp(phase,'Pdiff'))&(~strcmp(phase,'Sdiff'))
   ...
   case {4}
      if isstr(dangle)
         ...
         %%% or target distance for Pdiff or Sdiff
         if (~strcmp(phase,'Pdiff'))&(~strcmp(phase,'Sdiff'))
             %%% phase is NOT Pdiff or Sdiff
             if length(dangle)==1
             ...
             %%% phase IS Pdiff or Sdiff
             angles=dangle; % make dangle the target distance
         end; % if (~strcmp(phase,'Pdiff'))&(~strcmp(phase,'Sdiff'))
      end; % if isstr(dangle)
   case {5}
      %%% p5 is a colspec
      colspec=str2num(p5);
...
for indy=1:angleanz
    ...
    %%% compute ray path
    switch phase
       case {'Pdiff','Sdiff'} % new handling mode MK12112010
           [dist,rx,rz,rt,resp]=mkx4p(phase,h,angle,model,'distance');
       otherwise
    ...
    if (~isnan(dist))&(~isinf(dist))
       ...
       %%% ray length
       raylen=mkraylength(rx,rz,model.rp);
       %%% annotate in object's user data
       set(newray,'UserData',...
           {'takeoff' num2str(angle);...
            'slo' num2str(resp);...
            'max(z)' num2str(max(rz));...
            'Delta' num2str(dist);...
            'Ray Length' num2str(raylen)});
    else
