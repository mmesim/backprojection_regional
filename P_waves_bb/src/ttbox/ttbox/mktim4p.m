             ...
             case {'angle'}
              ...
              case {'distance'} % MK12112010
                 ...
                 if (~strcmp(phase,'Pdiff'))&(~strcmp(phase,'Sdiff'))
                    error('MKTIM4P: if ANGLEMODE is DISTANCE, the phase has to be Pdiff or Sdiff!');
                 end; % if
...
%                   %%% end of trial fix MK01122006
   ...
   case{'Pdiff','Sdiff'}  %% MK12112010
      
      %%% compute vertex depth
      vdep=modelsav.rp-modelsav.cmb; % vdep is a radius!
   
      %%% switch between Pdiff and Sdiff by choosing the velocity
      %%% determine velocity above CMB
      abovecmb=mkinterpmodel(modelsav,modelsav.cmb,'simple');
      switch phase
         case {'Pdiff'}
              vflat=vpflat;
              cmbvelocity=abovecmb.vp(1);
         case {'Sdiff'}
              vflat=vsflat;
              cmbvelocity=abovecmb.vs(1);
...
%             end; % if vdepflat
         ...
         %%% compute traveltime from surface to real focus at HFLAT
         tt2=mktpsum(p,vflat,zflat,0,hflat,1); % novertex=1: suppress vertex code
         
         %%% travel time along CMB
         %%% the only way to determine the leg length along the CMB is to
         %%% call mkx4p for the P or S wave with the same ray parameter...
         
         cmbpathlen=targetdistance-basedist; % degrees
         cmbpathlen=2*pi*vdep*cmbpathlen/360; % km
         cmbtt=cmbpathlen/cmbvelocity; % travel time along CMB leg
         ...
         if takeoff<=90 % MK11102006, was: takeoff<90
            ...
            vdepflat=dmyz;
            %%% compute traveltime for surface focus
            tt1=mktpsum(p,vflat,zflat,0,vdepflat,0);
         
            %%% total traveltime of phase is twice the time for surface focus
            %%% minus time from surface to focal depth
            %%% plus time along CMB leg
            tt=2*tt1-tt2+cmbtt;
