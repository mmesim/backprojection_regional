...
%
% Martin Knapmeyer 03.07.2002, 05.07.2002, 29.08.2003, 20.10.2003,
%                  29.10.2003, 12.04.2010, 09.12.2010
%
...
if ~silent
   disp(['MKTTCURVES: computing traveltime for h= ' num2str(h) 'km:']);
end; % if ~silent
...
if ~isempty(pphases)
	for indy=1:panz
       ...
       if length(dangle)==1
          ...
          if isempty(dangle)
             defaultdangle=0.5;
             angles=mksmarttakeoff(phase,model,h,defaultdangle);
          else
             angles=dangle;
       ...
       switch phase
          case {'Pdiff','Sdiff'}
             %%% Pdiff and Sdiff require extra handling because of their
             %%% ray parameter independence MK22122010
             
             %%% determine minimum distance possible for Pdiff and Sdiff
             switch lower(phase(1))
                case {'s'}
                     diffmindist=mkx4p('S',h,angles,model,'angle');
                case {'p'}
                     diffmindist=mkx4p('P',h,angles,model,'angle');
                otherwise
                   error(['MKTTCURVES: unknown phase name ' upper(phase)]);
             end; % switch
             
             %%% construct distance list
             distances=diffmindist:defaultdangle:180;
             
             %%% loop over all distances
             for cnt=1:length(distances)
                ...
                if length(vstart)>1 % MK12042010
                   if angles<90 
                      %%% downgoing ray
                      vstart=vstart(end); % take velocity underneath discontinuity
                   else
                      %%% upgoing ray
                      vstart=vstart(1); % take velocity above discontinuity
                   end; % if angles
                end; % if length(vstart>1)
                p(cnt)=radian*sin(angles*radian)*(model.rp-h)/vstart;
                %disp(num2str([angle p]));

                %%% evaluate model for ray geometry and travel time
                [dst,prx,prz,pty]=mkx4p(phase,h,distances(cnt),model,'distance');
                currenttt=mktim4p(phase,h,distances(cnt),model,'distance');

                %%% store result
                dist(cnt)=distances(cnt);
                rayp(cnt)=p(cnt);
                angle(cnt)=angles;
                tt(cnt)=currenttt;
                pendep(cnt)=max(prz);
          ...
          otherwise
             %%% standard handling for all other phases
             for cnt=1:length(angles)
                ...
                %%% evaluate model for distance and travel time
                [dst,prx,prz,pty]=mkx4p(phase,h,p(cnt),model,angles(cnt));%dist(cnt)=mkx4p(phase,h,p(cnt),model);
                currenttt=mktim4p(phase,h,p(cnt),model,angles(cnt));
       ...
       if makefigs(3)==1
          ...
          %%% add text label at travel time curve line
          %%% the following takes a rndom position along the travel time
          %%% curve, to statistically avoid label overlappings.
          %%% just another idea: place the label at the middle of the
          %%% curve. How would that look?
          %notnan=find((~isinf(dist))&(~isinf(tt))&(~isnan(dist))&(~isnan(tt)));
          %labelnr=round(rand(1,1)*(length(notnan)-1))+1;
          %text(mkreduceto180(dist(notnan(labelnr))),tt(notnan(labelnr)),phase);
       end; % if makefigs
...
%        figure(3);
%        distances=80:10:180;
%        pdifftt=zeros(size(distances));
%        sdifftt=zeros(size(distances));
%        hold on;
%        for index=1:length(distances)
%           pdifftt(index)=mktim4p('Pdiff',h,distances(index),model,'distance');
%        end;   
%        plot(distances,pdifftt,'+m--');
%        text(mean(distances),pdifftt(round(length(distances)/2)+1),'Pdiff');
% 
%        for index=1:length(distances)
%           sdifftt(index)=mktim4p('Sdiff',h,distances(index),model,'distance');  
%        end;
%        plot(distances,sdifftt,'ok--');
%        text(mean(distances),sdifftt(round(length(distances)/2)+1),'Sdiff');
%        hold off;     
%   end; % if makefigs(3)==1
% %%% PDiff and SDiff complete
...
if makefigs(3)==1
   ...
   %%% add matlab axes without labels to enable grid
   axis on
   axis tight
   set(gca,'XTickLabel',{});
   set(gca,'YTickLabel',{});
