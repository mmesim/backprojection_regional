...
%
% Martin Knapmeyer, 10.04.2002, 04.09.2003, 17.09.2008, 01.11.2011
...
if fid==-1
   ...
   fprintf(fid,'!name %s\n',name);
   fprintf(fid,'!year %s\n',int2str(year)); % int2str inserted MK17092008
   fprintf(fid,'!radius %s\n',num2str(radius,'%7.3f')); % num2str inserted MK01112011
   ...
   for indy=1:anz
      ...
      if writtend<=length(dz)
         if abs(z(indy)-dz(writtend))<1e-3 %z(indy)==dz(writtend) % writtend may have illegal values here, so do not combine the IFs!
